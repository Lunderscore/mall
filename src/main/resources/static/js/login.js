$(function(){
    ...
    $("#submitBtn").on("click", function(){
        ...
        if ($("#loginForm").attr("action") == "login" && checkLogin()){
            $.post(APP_PATH + "/loglogin", {userUsername:username, userPassword:password1 }, function(data){
                if (data.code == 0){
            ...
            $.get(APP_PATH + "/users/"+username, function(data){
                if (data.code == 1){
                    alert("存在相同用户名，请换其他用户名！");
                }else{
                    $.post(APP_PATH + "/regregister", {userUsername:username, userPassword:password1 }, function(data){
                        location.href = APP_PATH + "/index";
                    });
                }
            });
